...
var a = {
  ...
  , o = (...t)=>{
    a.log.innerText += t.join(" ") + `
`,
    console.log(...t)
}
  , i = t=>a.fps.innerText = t
  , g = t=>a.perf.innerText = "tensors:" + e.tf.memory().numTensors.toString() + " | performance: " + JSON.stringify(t).replace(/"|{|}/g, "").replace(/,/g, " | ");
async function f() {
    if (!a.video.paused) {
        n.start === 0 && (n.start = e.now()),
        await e.detect(a.video);
        ...
        s.frames % 100 === 0 && !a.video.paused && o("performance", {
            ...s,
            tensors: n.tensors
        })
    }
